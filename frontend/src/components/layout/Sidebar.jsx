...
export function Sidebar() {
  ...
  return (
    <aside className="sidebar hidden lg:flex fixed left-0 top-0 bottom-0 w-[260px] flex-col z-50">
      {/* Logo */}
        ...
        {navItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <NavLink
              key={to}
              to={to}
              className={`sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-[14px] font-medium transition-all ${active ? 'active' : ''}`}
            >
